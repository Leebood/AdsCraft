...
AIGC:
    ...
    PropagateID: 3476033405197864#1782551695354
    ReservedCode2: ""
---
# AdsCraft 首页改版方案 V2
...
│  ② Reduce audience overlap — 3 ad sets     │
│  ③ Add hook in first 3 seconds of video    │
...
### 动态2：Report 分数动画
- 位置：第三屏（Sample Report）+ Facebook诊断结果页 + TikTok诊断结果页
- 三处共用同一个 `ScoreAnimation` 组件，首页用mock数据，诊断结果页用真实数据
- 触发：
  - 首页：滚动到该屏时触发（Intersection Observer）
  - 诊断结果页：报告生成后自动触发
- 效果：
  - Overall Score 从0增长到最终值
  - 各维度进度条从0填充
  - Next Actions 逐条淡入（每条间隔0.3秒）
- 时长：总计2-3秒
- 首页只触发一次；诊断结果页每次都播放（用户每次做完诊断都会看到）
